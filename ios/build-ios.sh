...
# Create unsigned IPA
APP_PATH=$(find "$BUILD_DIR/game" -name "LawnProject.app" -path "*${BUILD_TYPE}*" | head -1)
if [ -z "$APP_PATH" ]; then
    ...
    cd "$IPA_DIR"
    zip -r -y "$BUILD_DIR/LawnProject-ios.ipa" Payload/
    rm -rf "$IPA_DIR"
    echo "IPA created: $BUILD_DIR/LawnProject-ios.ipa"
fi
